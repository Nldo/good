...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        img2 = F.interpolate(img.unsqueeze(0), size=(64), mode='bilinear', align_corners=False)
        img2 =img2.squeeze(0)
        # print(label.max())
        label2 = torch.from_numpy(cv2.resize(label.cpu().numpy().squeeze().astype("float"), (64, 64))).unsqueeze(0)

        return {'name': name, 'A': img,'A2': img2, 'B': img_B, 'L': label,'L2': label2}
