...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        if args.optimizer == "sgd":
            ...
            self.optimizer_G = optim.Adam(self.net_G.parameters(), lr=self.lr,weight_decay=1e-4)
        ...
        if args.loss == 'ce':
            ...
            self._pxl_loss = losses.hybrid_loss
        else:
    ...
    def _forward_pass(self, batch):
        ...
        self.G_pred,self.G_pred2,self.G_pred3,self.G_pred4 = self.net_G(img_in1, img_in2)
    ...
    def _backward_G(self):
        ...
        gt2 = self.batch['L2'].to(self.device).float()
        gt3 = self.batch['B'].to(self.device).float()
        gt4 = self.batch['A2'].to(self.device).float()
        self.loss1 = self._pxl_loss(0.5).cuda()
        G_loss1 = self.loss1(self.G_pred, gt)
        # G_loss2 = self.loss1(self.G_pred2, gt2)
        G_loss2 = self.loss1(self.G_pred2, gt2)
        G_loss3 = self.loss2(self.G_pred3, gt3)
        G_loss4 = self.loss2(self.G_pred4, gt4)
        self.G_loss = 1*G_loss1 + 0.5*G_loss2 + 0.1 * G_loss3 + 0.05 * G_loss4
        # self.G_loss =G_loss3
